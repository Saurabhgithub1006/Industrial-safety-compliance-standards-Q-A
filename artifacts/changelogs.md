...
## CHG-20260911-07 — Phase 5: Judge 2 escalation + HITL review — 2026-09-11

**What:** Built Phase 5: Judge 2 packages Judge 1's non-`supported` claims into severity-ranked escalation packets, a persistent queue holds them for human review, a CLI lets a reviewer approve/edit/reject each with a required rationale, and final-answer assembly combines `supported` claims with HITL-cleared ones — pending and rejected claims are held back.

**How it works:** `review/escalation.py` builds one `EscalationPacket` per escalated claim, sorted worst-first (`contradicted` outranks `unsupported`) — deliberately zero LLM calls (an optional LLM-written packet summary was considered and explicitly deferred; severity ranking and deduping are mechanical, not judgment calls). `review/store.py` persists the queue in a database file **separate** from `corpus.db` — the queue holds real human-decision history that must never share a file with something `run_ingest` can rebuild — with dedup-on-insert (a claim escalated again from a later query, same clause + same claim text, merges into the existing pending item rather than duplicating it, but only while that item is still pending; an already-decided item doesn't silently absorb a fresh recurrence of the same issue). `review/cli.py` is the interactive reviewer loop, with its decision-applying logic split out (`apply_decision`) so it's unit-testable without simulating stdin. `review/assembly.py` combines `judged.supported_claims` with items whose `review_item.status` is `approved` or `edited` (an edit's corrected text/quote/clause overrides the original — a human-edited claim is not re-run through Judge 1's automated check, the human review is the final check for that claim); `pending`/`rejected` items are excluded and counted separately. `review/pipeline.py` wires generation → Judge 1 → Judge 2 → queue → assembly into one command.

**Result:** 16 new tests, all deterministic (this phase makes no LLM calls at all, so nothing needed a fake or real client). Validated live end to end, not just against fakes: a deliberately wrong claim ("inspect monthly" vs. the clause's actual "at least annually") was run through a real Kimi Judge 1 call, correctly verdicted `contradicted`; Judge 2 built the escalation packet; the claim was correctly excluded from the final answer before review (`pending_count=1`) and correctly excluded again after a simulated reject decision (`rejected_count=1`). 71 tests passing total (55 pre-existing + 16 new).

**Files:** `src/safety_qa/review/escalation.py`, `store.py`, `assembly.py`, `cli.py`, `pipeline.py`, `tests/test_review.py`, `README.md`

---

## CHG-20260911-08 — Phase 6: feedback loop and regression harness — 2026-09-11

**What:** Built Phase 6: extract every case where a human overturned Judge 1 (approved or edited a flagged claim) into a judge-override-rate metric, and replay each one through a fresh Judge 1 call to catch prompt/model drift.

**How it works:** `review/regression.py`'s `overturned_cases()` reads `review_item` rows with status `approved`/`edited` and their latest `review_decision` row; `override_rate()` computes overturned/(overturned+confirmed) across all decided items. Both are pure functions, no LLM calls. `run_regression.py` replays each overturned case through a live `GroundingJudge` call and reports whether it's still wrongly flagged (`REGRESSION`) or now correctly `supported` (`ok`) — but only for cases whose original verdict came from Judge 1's own LLM judgment (`judge1_source == "llm_judge"`); a case caught by the deterministic layer (hallucinated citation, non-verbatim quote) is a pure string/lookup check that reproduces identically forever unless the corpus itself changes, so replaying it tests nothing and would waste an API call.

**Why the `judge1_source` field exists:** it wasn't in the original Phase 6 plan. Building this against real review history (see below) surfaced it directly: the first version of `run_regression.py` replayed an overturned case that had actually been caught by the deterministic layer, and it reproduced the identical `unsupported` result every time — correctly, but the code labeled it `REGRESSION`, which is misleading for a check that was never capable of "drifting" in the first place. `judge1_source` was added to `EscalationPacket`/`review_item` (with a schema migration for the queue database that already existed with the old schema) so the replay logic can tell the two cases apart.

**Result:** Seeded with real review history to build and validate this against (per the Phase 6 kickoff decision, Option A): 3 organic real questions run through the live pipeline produced zero escalations — Judge 1 and the generator were already precise enough that nothing organic was available to seed with, itself an honest, positive signal. 3 deliberately-wrong claims were then run through the real pipeline instead (same transparent technique as CHG-20260911-07's smoke test): 2 were genuine contradictions correctly caught by Judge 1 (confirmed via reject), 1 was a genuine transcription error in the quote (the underlying claim was true; fixed via edit) — giving a real, non-fabricated override rate of 33% (1/3) and one genuine overturned case, which turned out to be the deterministic-layer case that led to the `judge1_source` fix above. 6 new tests, all deterministic. 77 tests passing total (71 pre-existing + 6 new).

**Files:** `src/safety_qa/review/regression.py`, `run_regression.py`, `escalation.py` (added `judge1_source`), `store.py` (added `judge1_source` column + migration), `tests/test_regression.py`, `README.md`

---

## CHG-20260912-09 — Phase 7: hardening — 2026-09-12

**What:** Scoped Phase 7 to the 3 items the roadmap actually names: observability (log every Judge 1 verdict with model/prompt version), reviewer identity (no silent anonymous default), and a `wait_for_review` opt-in.

**How it works:** `judging/prompt.py` gets a `JUDGE_PROMPT_VERSION` constant to bump when the judge prompt's wording changes meaningfully. `JudgedClaim` gets `model`/`prompt_version` fields, populated from `self.llm.model` (added as a declared attribute on the `LLMClient` protocol, and on `FakeLLMClient`, which previously had none) for `llm_judge`-sourced verdicts, left `None` for `deterministic_check`-sourced ones (no model was called, so there's nothing to tag). A new `judge_verdict_log` table in `review_queue.db` (matching the arch doc's `JudgeVerdict` data model) records every verdict via `store.log_verdicts()` — the only permanent record a `supported` verdict ever gets, since `review_item` only ever held escalated claims. `review/cli.py`'s `resolve_reviewer_id()` requires `--reviewer NAME` or keeps prompting until given a non-empty identity — no more shared `"local-reviewer"` default. `review/pipeline.py` gets a `--wait` flag: reviews any claims it just escalated inline, in the same run (reusing `cli.py`'s decision loop, factored out as `review_items_interactively()` rather than duplicated), before assembling and printing the final answer.
